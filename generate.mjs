...
async function main() {
  try {
    ...
    fs.mkdirSync(config.outputFolderJson, { recursive: true });
    ...
    const { machineStrength } = await inquirer.prompt({
      ...
      message: chalk.cyan(
        "What level of processing intensity do you want to run this task with? (Choose a lower level for weaker machines)"
      ),
      choices: [
        ...
        {
          name: "Very High (8 - Suitable for very high-end machines)",
          value: 8,
        },
        {
          name: "Ultra (10 - Suitable for top-tier hardware specs)",
          value: 10,
        },
        {
          name: "Insane (12 - Only for the most powerful machines)",
          value: 12,
        },
      ],
    });
    ...
    if (config.numImages > totalCombinations) {
      logError(
        `Requested number of files (${config.numImages}) exceeds the total possible combinations (${totalCombinations}). Please lower the number of files to generate.`
      );
      return;
    }
...
async function generateImage(i, config) {
  try {
    ...
    const chosenTraits = await generateUniqueTraits(
      config.traitFolders,
      config
    );
    ...
    const outputJsonPath = path.join(
      config.outputFolderJson,
      `${i + config.startAt}.json`
    );
...
async function generateGif(i, config) {
  try {
    ...
    for (let j = 0; j < config.traitFolders.length; j++) {
      ...
      if (!frames) {
        console.error(
          `Failed to extract frames from file at path: ${chosenTraitPath}`
        );
        return;
      }
